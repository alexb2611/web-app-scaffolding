...
> **The `Note` resource referenced throughout this doc is real**, not hypothetical — it ships in the scaffold as the canonical example. Every numbered step has a "✅ Reference" line pointing at the actual file you can copy the shape of. Snippets in this doc are abbreviated; the live files are the source of truth.
...
> ✅ **Reference:** [`backend/app/models/note.py`](../backend/app/models/note.py) — the live Note model in this scaffold.
...
> ✅ **Reference:** [`backend/app/models/__init__.py`](../backend/app/models/__init__.py) — note how `Note` is imported alongside `User` and `RefreshToken`.
...
> ✅ **Reference:** [`backend/alembic/versions/59c8b574b17d_add_notes_table.py`](../backend/alembic/versions/59c8b574b17d_add_notes_table.py) — autogenerated, no hand edits required.

## 4. Pydantic schemas

`backend/app/schemas/note.py` (new file). Pattern: see `backend/app/schemas/auth.py`.

> ✅ **Reference:** [`backend/app/schemas/note.py`](../backend/app/schemas/note.py) — `NoteCreate`, `NoteUpdate`, `NoteResponse`.
...
> ✅ **Reference:** [`backend/app/services/note_service.py`](../backend/app/services/note_service.py) — note the authorisation-baked-into-the-query pattern (`get_note_for_user`).
...
> ✅ **Reference:** [`backend/app/api/v1/notes.py`](../backend/app/api/v1/notes.py) — all five CRUD endpoints. Note the consistent 404-not-403 invariant.
...
> ✅ **Reference:** [`backend/tests/test_notes.py`](../backend/tests/test_notes.py) — 13 tests covering auth-required, CRUD round trips, cross-user 404 invariant, and validation edges.
...
> ✅ **Reference:** [`frontend/src/lib/note-schemas.ts`](../frontend/src/lib/note-schemas.ts) — including the `_AssertCreate extends ... ? true : never` compile-time alignment check.
...
> ✅ **Reference:** [`frontend/src/app/notes/page.tsx`](../frontend/src/app/notes/page.tsx) — list + create + delete, with the optimistic-prepend pattern and the typed `client.DELETE("/api/v1/notes/{note_id}", { params: { path: { note_id } } })` call.
...
> ✅ **Reference:** [`frontend/src/lib/note-schemas.test.ts`](../frontend/src/lib/note-schemas.test.ts) — 4 tests covering title/body required and length bounds.
...
> ✅ **Reference:** [`frontend/e2e/notes.spec.ts`](../frontend/e2e/notes.spec.ts) — auth-gate, create + list, ordering, delete, and inline-validation specs.
